...
if [[ $(xrandr --listactivemonitors | grep -v "Monitors" | cut -d" " -f4 | cut -d"+" -f2- | uniq | wc -l) == 1 ]]; then
  ...
  MONITOR=$SCREEN polybar --config=$CONFIG primary 2>&1 | tee -a /tmp/polybar_$m.log & disown
# Handle multiple monitor layout
  ...
  for SCREEN in $SCREENS; do
    ...
    # Handle secondary monitors
      ...
      MONITOR=$SCREEN polybar --config=$CONFIG secondary 2>&1 | tee -a /tmp/polybar_$m.log & disown
    fi
  done
fi
